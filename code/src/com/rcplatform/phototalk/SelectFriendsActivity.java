...
public class SelectFriendsActivity extends BaseActivity implements
		...
		Thread th = new Thread() {
			public void run() {
				...
				for (Friend friend : friends) {
					SelectFriend seleFriend = SelectFriend
							.parseSelectFriend(friend);
				...
				user.setLetter(RCPlatformTextUtil.getLetter(getCurrentUser()
						.getNickName()));
				seleFriends.add(user);
				mHandler.obtainMessage(MSG_CACHE_FINISH, seleFriends)
						.sendToTarget();
				getFriends();
			};
		};
	...
	private void initViewOrListener() {
		...
		mGallery.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				list.get(sendData.get(position).getPosition()).setIsChosed(
						false);
	...
	private void initFriendListAdapter(List<SelectFriend> list) {
		...
		adapter.setOnCheckBoxChangedListener(new OnCheckBoxChangedListener() {
			...
			@Override
			public void onChange(SelectFriend friend, boolean isChecked,
					int index) {
	...
	private void jsonToFriends(String json) throws JSONException {
		...
		List<Friend> friends = JSONConver.jsonToFriends(myFriendsArray
				.toString());
		for (Friend friend : friends) {
	...
	private void sendPicture(String imagePath, final String timeLimit,
			final List<SelectFriend> friends) {
