...
public class PhotoTalkInformationStateService {

	private static PhotoTalkInformationStateService service;

	private PhotoTalkInformationStateService() {

	}

	public static PhotoTalkInformationStateService getInstence() {
		if (service == null) {
			service = new PhotoTalkInformationStateService();
		}
		return service;
	}
...
//		GalHttpRequest request = GalHttpRequest.requestWithURL(context, url);
		...
		if (params != null)
			client.putAllRequestParams(params);
		client.post(context, url, null);
	}

	public void postRequestByTimestamp(Context context, PhotoChatHttpLoadTextCallBack callBack, Map<String, String> params, String url, long time, byte[] data) {
