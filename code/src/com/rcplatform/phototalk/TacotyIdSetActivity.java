...
public class TacotyIdSetActivity extends Activity implements View.OnClickListener {
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
				case MenueApiFactory.RESPONSE_STATE_SUCCESS:
					...
					data.putExtra("result", mTacotyIdEidtView.getText().toString());
					setResult(RESULT_OK, data);
					finish();
					break;
				case RESPONSE_STATE_NOT_AVAILABLE_TACOTYID:
					showDialog(TacotyIdSetActivity.this, getResources().getString(R.string.setting_update_not_available_tacotyid));
					break;
			}
		}
	...
	private EditText mTacotyIdEidtView;

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.setting_update_tacotyid_activity);
		mBack = findViewById(R.id.back);
		mBack.setVisibility(View.VISIBLE);
		mBack.setOnClickListener(this);
		mTitleTextView = (TextView) findViewById(R.id.titleContent);
		mTitleTextView.setVisibility(View.VISIBLE);
		mTitleTextView.setText(getResources().getString(R.string.setting_update_set_tacotyid_title));

		mTacotyIdEidtView = (EditText) findViewById(R.id.set_tacoty_id);
	...
	public void onClick(View v) {
		switch (v.getId()) {
			...
			case R.id.settings_update_confirm_button:
				final String label = mTacotyIdEidtView.getText().toString();
				// 请输入TacotyID
				if (TextUtils.isEmpty(label)) {
					showDialog(this, getResources().getString(R.string.setting_update_set_tacoty_id));
					return;
				}
				// 你是否要创建ID xxxxxx？创建后无法更改。
				String title = getResources().getString(R.string.setting_update_create_tacoty_id);
				title = String.format(title, label);
				AlertDialog.Builder dialogBuilder = new AlertDialog.Builder(this);
				dialogBuilder.setMessage(title).setCancelable(false).setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {

					@Override
					public void onClick(DialogInterface dialog, int which) {
						mProgressbar.setVisibility(View.VISIBLE);
						createTacotyId(label);
					}
