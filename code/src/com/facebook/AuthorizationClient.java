...
class AuthorizationClient implements Serializable {
	...
	class WebViewAuthHandler extends AuthHandler {
		...
		boolean tryAuthorize(final AuthorizationRequest request) {
			...
			WebDialog.Builder builder = new AuthDialogBuilder(getStartActivityDelegate().getActivityContext(), applicationId, parameters)
	...
	class KatanaLoginDialogAuthHandler extends KatanaAuthHandler {
		...
		boolean tryAuthorize(AuthorizationRequest request) {
			Intent intent = NativeProtocol.createLoginDialog20121101Intent(context, request.getApplicationId(),
			                                                               new ArrayList<String>(request.getPermissions()), request
	...
	static class AuthorizationRequest implements Serializable {
		...
		AuthorizationRequest(SessionLoginBehavior loginBehavior, int requestCode, boolean isLegacy, List<String> permissions,
		                     SessionDefaultAudience defaultAudience, String applicationId, String validateSameFbidAsToken,
